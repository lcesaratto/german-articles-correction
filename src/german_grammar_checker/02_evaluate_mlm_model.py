...
class BertForMaskedLM:
    ...
    def _get_dataloader(self, data_path):
        ...
        dataloader = self._create_dataloader(input_ids, attention_masks, labels)
    ...
    def predict(self, test_data_path):
        ...
        for idx, batch in enumerate(test_dataloader):
            ...
            for i in range(logits.shape[0]):
                mask_token_index = (parameters["input_ids"] == self.tokenizer.mask_token_id)[i].nonzero(as_tuple=True)[0]
                predicted_token_id = logits[i, mask_token_index].argmax(axis=-1)
                print(pd.read_csv(test_data_path, skiprows=range(1,idx+i+1), nrows=1)["masked_sentence"][0])
                print("PREDICTED: ", self.tokenizer.decode(predicted_token_id))
                print("EXPECTED: ", self.tokenizer.decode(batch[2][i].to(self.device)))


bert_for_masked_lm = BertForMaskedLM(MODEL_NAME, BATCH_SIZE)
