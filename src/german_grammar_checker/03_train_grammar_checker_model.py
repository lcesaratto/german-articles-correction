...
class BertForGrammarCorrectionTrainer:
    def __init__(self, model_name):
        ...
        self.model_class.init_model_and_optimizer()
    ...
    def train_model_on_full_train_data(self, batch_size, num_epochs, train_data_path):
        ...
        train_dataloader = DataPreparator.get_dataloaders(train_data_path, self.model_name, batch_size)
        ...
        try:
            for epoch in range(num_epochs):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    outputs = self.model_class.model(**parameters)

                    loss = outputs.loss
                    total_train_loss += loss.item()
                    loss.backward()

                    torch.nn.utils.clip_grad_norm_(self.model_class.model.parameters(), 1.0)
...
bert_for_checking_grammar = BertForGrammarCorrectionTrainer(MODEL_NAME, BATCH_SIZE, NUM_EPOCHS)
bert_for_checking_grammar.train_model_on_full_train_data(TRAIN_DATA_PATH)
bert_for_checking_grammar.save_model_state_dict(PRETRAINED_MODEL_PATH)
